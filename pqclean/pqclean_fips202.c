 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for PQCLEAN_SHAKE128)
...
void pqclean_shake128_inc_absorb(pqclean_shake128incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, PQCLEAN_SHAKE128_RATE, input, inlen);
}

void pqclean_shake128_inc_finalize(pqclean_shake128incctx *state) {
    keccak_inc_finalize(state->ctx, PQCLEAN_SHAKE128_RATE, 0x1F);
}

void pqclean_shake128_inc_squeeze(uint8_t *output, size_t outlen, pqclean_shake128incctx *state) {
    keccak_inc_squeeze(output, outlen, state->ctx, PQCLEAN_SHAKE128_RATE);
}
...
void pqclean_shake256_inc_absorb(pqclean_shake256incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, PQCLEAN_SHAKE256_RATE, input, inlen);
}

void pqclean_shake256_inc_finalize(pqclean_shake256incctx *state) {
    keccak_inc_finalize(state->ctx, PQCLEAN_SHAKE256_RATE, 0x1F);
}

void pqclean_shake256_inc_squeeze(uint8_t *output, size_t outlen, pqclean_shake256incctx *state) {
    keccak_inc_squeeze(output, outlen, state->ctx, PQCLEAN_SHAKE256_RATE);
}
...
/*************************************************
 * Name:        pqclean_shake128_absorb
 *
 * Description: Absorb step of the PQCLEAN_SHAKE128 XOF.
 *              non-incremental, starts by zeroeing the state.
 *
 * Arguments:   - uint64_t *s: pointer to (uninitialized) output Keccak state
...
void pqclean_shake128_absorb(pqclean_shake128ctx *state, const uint8_t *input, size_t inlen) {
    ...
    keccak_absorb(state->ctx, PQCLEAN_SHAKE128_RATE, input, inlen, 0x1F);
}

/*************************************************
 * Name:        pqclean_shake128_squeezeblocks
 *
 * Description: Squeeze step of PQCLEAN_SHAKE128 XOF. Squeezes full blocks of
 *              PQCLEAN_SHAKE128_RATE bytes each. Modifies the state. Can be called
 *              multiple times to keep squeezing, i.e., is incremental.
 *
 * Arguments:   - uint8_t *output: pointer to output blocks
 *              - size_t nblocks: number of blocks to be squeezed
 *                                            (written to output)
 *              - pqclean_shake128ctx *state: pointer to input/output Keccak state
 **************************************************/
void pqclean_shake128_squeezeblocks(uint8_t *output, size_t nblocks, pqclean_shake128ctx *state) {
    keccak_squeezeblocks(output, nblocks, state->ctx, PQCLEAN_SHAKE128_RATE);
}
...
/*************************************************
 * Name:        pqclean_shake256_absorb
 *
 * Description: Absorb step of the PQCLEAN_SHAKE256 XOF.
 *              non-incremental, starts by zeroeing the state.
 *
 * Arguments:   - pqclean_shake256ctx *state: pointer to (uninitialized) output Keccak state
 *              - const uint8_t *input: pointer to input to be absorbed
 *                                            into s
 *              - size_t inlen: length of input in bytes
 **************************************************/
void pqclean_shake256_absorb(pqclean_shake256ctx *state, const uint8_t *input, size_t inlen) {
    ...
    keccak_absorb(state->ctx, PQCLEAN_SHAKE256_RATE, input, inlen, 0x1F);
}

/*************************************************
 * Name:        pqclean_shake256_squeezeblocks
 *
 * Description: Squeeze step of PQCLEAN_SHAKE256 XOF. Squeezes full blocks of
 *              PQCLEAN_SHAKE256_RATE bytes each. Modifies the state. Can be called
 *              multiple times to keep squeezing, i.e., is incremental.
 *
 * Arguments:   - uint8_t *output: pointer to output blocks
 *              - size_t nblocks: number of blocks to be squeezed
 *                                (written to output)
 *              - pqclean_shake256ctx *state: pointer to input/output Keccak state
 **************************************************/
void pqclean_shake256_squeezeblocks(uint8_t *output, size_t nblocks, pqclean_shake256ctx *state) {
    keccak_squeezeblocks(output, nblocks, state->ctx, PQCLEAN_SHAKE256_RATE);
}
...
/*************************************************
 * Name:        pqclean_shake128
 *
 * Description: PQCLEAN_SHAKE128 XOF with non-incremental API
 *
...
void pqclean_shake128(uint8_t *output, size_t outlen,
    ...
    size_t nblocks = outlen / PQCLEAN_SHAKE128_RATE;
    uint8_t t[PQCLEAN_SHAKE128_RATE];
    pqclean_shake128ctx s;

    pqclean_shake128_absorb(&s, input, inlen);
    pqclean_shake128_squeezeblocks(output, nblocks, &s);

    output += nblocks * PQCLEAN_SHAKE128_RATE;
    outlen -= nblocks * PQCLEAN_SHAKE128_RATE;

    if (outlen) {
        pqclean_shake128_squeezeblocks(t, 1, &s);
        for (size_t i = 0; i < outlen; ++i) {
            output[i] = t[i];
        }
    }
    pqclean_shake128_ctx_release(&s);
}

/*************************************************
 * Name:        pqclean_shake256
 *
 * Description: PQCLEAN_SHAKE256 XOF with non-incremental API
 *
 * Arguments:   - uint8_t *output: pointer to output
 *              - size_t outlen: requested output length in bytes
 *              - const uint8_t *input: pointer to input
...
void pqclean_shake256(uint8_t *output, size_t outlen,
              const uint8_t *input, size_t inlen) {
    size_t nblocks = outlen / PQCLEAN_SHAKE256_RATE;
    uint8_t t[PQCLEAN_SHAKE256_RATE];
    pqclean_shake256ctx s;

    pqclean_shake256_absorb(&s, input, inlen);
    pqclean_shake256_squeezeblocks(output, nblocks, &s);

    output += nblocks * PQCLEAN_SHAKE256_RATE;
    outlen -= nblocks * PQCLEAN_SHAKE256_RATE;

    if (outlen) {
        pqclean_shake256_squeezeblocks(t, 1, &s);
        for (size_t i = 0; i < outlen; ++i) {
            output[i] = t[i];
        }
    }
    pqclean_shake256_ctx_release(&s);
}
...
void pqclean_sha3_256_inc_absorb(pqclean_sha3_256incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, PQCLEAN_SHA3_256_RATE, input, inlen);
}

void pqclean_sha3_256_inc_finalize(uint8_t *output, pqclean_sha3_256incctx *state) {
    uint8_t t[PQCLEAN_SHA3_256_RATE];
    keccak_inc_finalize(state->ctx, PQCLEAN_SHA3_256_RATE, 0x06);

    keccak_squeezeblocks(t, 1, state->ctx, PQCLEAN_SHA3_256_RATE);

    pqclean_sha3_256_inc_ctx_release(state);
...
/*************************************************
 * Name:        pqclean_sha3_256
 *
 * Description: PQCLEAN_SHA3-256 with non-incremental API
 *
 * Arguments:   - uint8_t *output:      pointer to output
 *              - const uint8_t *input: pointer to input
 *              - size_t inlen:   length of input in bytes
...
void pqclean_sha3_256(uint8_t *output, const uint8_t *input, size_t inlen) {
    ...
    /* Absorb input */
    keccak_absorb(s, PQCLEAN_SHA3_256_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t, 1, s, PQCLEAN_SHA3_256_RATE);
...
void pqclean_sha3_384_inc_absorb(pqclean_sha3_384incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, PQCLEAN_SHA3_384_RATE, input, inlen);
}
...
void pqclean_sha3_384_inc_finalize(uint8_t *output, pqclean_sha3_384incctx *state) {
    uint8_t t[PQCLEAN_SHA3_384_RATE];
    keccak_inc_finalize(state->ctx, PQCLEAN_SHA3_384_RATE, 0x06);

    keccak_squeezeblocks(t, 1, state->ctx, PQCLEAN_SHA3_384_RATE);

    pqclean_sha3_384_inc_ctx_release(state);
...
void pqclean_sha3_384(uint8_t *output, const uint8_t *input, size_t inlen) {
    ...
    /* Absorb input */
    keccak_absorb(s, PQCLEAN_SHA3_384_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t, 1, s, PQCLEAN_SHA3_384_RATE);
...
void pqclean_sha3_512_inc_absorb(pqclean_sha3_512incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, PQCLEAN_SHA3_512_RATE, input, inlen);
}
...
void pqclean_sha3_512_inc_finalize(uint8_t *output, pqclean_sha3_512incctx *state) {
    uint8_t t[PQCLEAN_SHA3_512_RATE];
    keccak_inc_finalize(state->ctx, PQCLEAN_SHA3_512_RATE, 0x06);

    keccak_squeezeblocks(t, 1, state->ctx, PQCLEAN_SHA3_512_RATE);

    pqclean_sha3_512_inc_ctx_release(state);
...
/*************************************************
 * Name:        pqclean_sha3_512
 *
 * Description: PQCLEAN_SHA3-512 with non-incremental API
 *
 * Arguments:   - uint8_t *output:      pointer to output
 *              - const uint8_t *input: pointer to input
 *              - size_t inlen:   length of input in bytes
...
void pqclean_sha3_512(uint8_t *output, const uint8_t *input, size_t inlen) {
    ...
    /* Absorb input */
    keccak_absorb(s, PQCLEAN_SHA3_512_RATE, input, inlen, 0x06);

    /* Squeeze output */
    keccak_squeezeblocks(t, 1, s, PQCLEAN_SHA3_512_RATE);
